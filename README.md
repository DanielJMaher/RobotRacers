...
**Design fully resolved; Phase 3 of the implementation catch-up complete (2026-08-20).** The docs (GDD, architecture, roadmap) describe the Tournament-bracket direction above in full — no open design questions remain, only balance/tuning numbers. The code is catching up in the roadmap's phased order; Phase 3 (the Tournament bracket core loop) is done. See [`docs/03-roadmap/roadmap.md`](docs/03-roadmap/roadmap.md) for what's next (Environment Interludes, Phase 4).
...
- `@chao-draft/sim`: a pure, DOM-free simulation core — the full type surface from [`data-schemas.md`](docs/02-technical/data-schemas.md) *(not yet updated for the Tournament pivot — still describes the old Generation/board shape; deliberately deferred, see the roadmap)*, a seeded RNG, all 5 stat colors plus Climb/Jump support, colorless Items, and Habitats as real card data (91 cards), a bonding rules engine (cumulative, unbounded bonding across 5 Body Regions with mixed positive/negative stat grants), a full draft engine (pack generation, bot drafting, a tick-by-tick pass-and-pick state machine), a Race Leg resolver supporting 5–8-leg procedurally generated courses across 7 Leg types, and a Tournament bracket engine (`tournament/`) — 24-entrant generation + scouting, a shared rank-a-field/eliminate-last-place primitive powering both the player's own interactive group and every silently-resolved off-path branch, and the full Round 1 → Round 2 → Round 3 → Final Race state machine with Evolution triggers at the two round-boundary milestones. Karate Bout is fully removed. 89 passing unit tests.
- `@chao-draft/app`: a playable Vite + React Tournament — draft a full 3-pack draft against 3 AI bot seats into your Chao's pool, then play an actual Tournament: bond/consume drafted cards between races, watch standings and scout any of the 24 entrants (click to reveal a fuzzy 1–5 stat read), run each group-stage race and see who's eliminated, and — if your Chao survives all 3 rounds — run the 10-leg Final Race for a 1st/2nd/3rd result. No Environment Interludes or Breeding yet (Phases 4/5).

Run `pnpm install`, then `pnpm build` / `pnpm typecheck` / `pnpm test` from the repo root, or `pnpm --filter @chao-draft/app dev` to play the current build. Next up per the roadmap: Environment Interludes (Phase 4).
