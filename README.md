...
**Design fully resolved; Phase 5.5 of the implementation catch-up complete (2026-08-20) — the core tournament-to-tournament loop is fully playable, with real Bond Card scarcity and race timing on top.** The docs (GDD, architecture, roadmap) describe the Tournament-bracket direction above in full — no open design questions remain, only balance/tuning numbers. See [`docs/03-roadmap/roadmap.md`](docs/03-roadmap/roadmap.md) for what's next (the rest of Phase 6's card-system follow-through — balance, deferred UI).
...
- `@chao-draft/sim`: a pure, DOM-free simulation core — the full type surface from [`data-schemas.md`](docs/02-technical/data-schemas.md) *(not yet updated for the Tournament pivot — still describes the old Generation/board shape; deliberately deferred, see the roadmap)*, a seeded RNG, all 5 stat colors plus Climb/Jump support, colorless Items, Habitats, and Seeds as real card data (96 cards), a bonding rules engine (cumulative, unbounded bonding across 5 Body Regions with mixed positive/negative stat grants, plus Awakening — 3 copies of a card fuse into one 3.5x-average-grant application), a full draft engine, a Race Leg resolver, a display-only race-timing layer (per-Leg + total times to the tenth of a second, calibrated to a real reference point), a Tournament bracket engine (24-entrant generation + scouting, a shared rank-a-field/eliminate-last-place primitive, the full Round 1 → Round 2 → Round 3 → Final Race state machine with Evolution triggers), an Environment engine (Habitat placement/2-star combining, Seed planting, pooled Fruit generation, an enforced splash tax, a solo 3-pack Environment Interlude Booster), and a Breeding engine (`tournament/breeding.ts`): tiered breeding-eligibility pools exactly matching GDD §6.4's exclusion rules, the flat 10%+10% stat-inheritance formula, and full Tournament-to-tournament roster generation (3 lineage babies + 21 fresh, every time). Karate Bout is fully removed. 126 passing unit tests.
- `@chao-draft/app`: a playable Vite + React Tournament that loops — draft, place Habitats, play a full Tournament (bonding with splash tax, Environment Interludes, scouting), and on reaching the Final Race, pick a breeding partner and watch all 3 finalists' pairings resolve; if you bred, your own baby becomes your starting Chao for a fresh Draft Booster and the cycle continues. **Bond Cards are one-time use** — bonding (or Awakening 3 copies together) spends the specific drafted card for good, same lifecycle as Habitat and Seed cards. **A real per-race Results screen** shows a ranked table with every Chao's total time and per-Leg breakdown after every race, not just a scrolling log. Elimination before the Final Race ends the run outright. Chao inspection/lineage UI (Name/Height/Weight/Record, ancestry) is deliberately deferred (Phase 6).

Run `pnpm install`, then `pnpm build` / `pnpm typecheck` / `pnpm test` from the repo root, or `pnpm --filter @chao-draft/app dev` to play the current build. Next up per the roadmap: the rest of Phase 6 — balance tuning and the deferred inspection/lineage UI and race-reward hooks.
