...
**Design fully resolved; Phase 5.6 of the implementation catch-up complete (2026-08-20) — the core tournament-to-tournament loop is fully playable, with real Bond/Potion Card scarcity, race timing, and color-blended Potions on top.** The docs (GDD, architecture, roadmap) describe the Tournament-bracket direction above in full — no open design questions remain, only balance/tuning numbers. See [`docs/03-roadmap/roadmap.md`](docs/03-roadmap/roadmap.md) for what's next (the rest of Phase 6's card-system follow-through — balance, deferred UI).
