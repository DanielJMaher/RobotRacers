...
**Design fully resolved; Phase 4 of the implementation catch-up complete (2026-08-20).** The docs (GDD, architecture, roadmap) describe the Tournament-bracket direction above in full — no open design questions remain, only balance/tuning numbers. The code is catching up in the roadmap's phased order; Phase 4 (Environment Interludes — Habitats, Fruit, Seeds) is done. See [`docs/03-roadmap/roadmap.md`](docs/03-roadmap/roadmap.md) for what's next (Breeding & tournament-to-tournament progression, Phase 5).
...
- `@chao-draft/sim`: a pure, DOM-free simulation core — the full type surface from [`data-schemas.md`](docs/02-technical/data-schemas.md) *(not yet updated for the Tournament pivot — still describes the old Generation/board shape; deliberately deferred, see the roadmap)*, a seeded RNG, all 5 stat colors plus Climb/Jump support, colorless Items, Habitats, and Seeds as real card data (96 cards), a bonding rules engine (cumulative, unbounded bonding across 5 Body Regions with mixed positive/negative stat grants), a full draft engine (pack generation, bot drafting, a tick-by-tick pass-and-pick state machine), a Race Leg resolver supporting 5–8-leg procedurally generated courses across 7 Leg types, a Tournament bracket engine (`tournament/`) — 24-entrant generation + scouting, a shared rank-a-field/eliminate-last-place primitive, the full Round 1 → Round 2 → Round 3 → Final Race state machine with Evolution triggers — and an Environment engine (Habitat placement/2-star combining, Seed planting, pooled Fruit generation, an enforced splash tax, and a solo 3-pack Environment Interlude Booster). Karate Bout is fully removed. 109 passing unit tests.
- `@chao-draft/app`: a playable Vite + React Tournament — draft a full 3-pack draft against 3 AI bot seats, place your drawn Habitat cards into your 3 Environment slots, then play an actual Tournament: bond/consume drafted cards (paying Fruit-funded splash tax where it applies), watch standings and scout any of the 24 entrants, run each group-stage race, pick through an Environment Interlude Booster after Round 1 and Round 2, plant Seeds into your Habitats, and — if your Chao survives all 3 rounds — run the 10-leg Final Race for a 1st/2nd/3rd result. No Breeding yet (Phase 5).

Run `pnpm install`, then `pnpm build` / `pnpm typecheck` / `pnpm test` from the repo root, or `pnpm --filter @chao-draft/app dev` to play the current build. Next up per the roadmap: Breeding & tournament-to-tournament progression (Phase 5).
