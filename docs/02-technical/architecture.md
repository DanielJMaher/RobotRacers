...
Requested directly by the user after a live playtest: habitat selection is confirmed open (any drawn Habitat card into any slot, any slot left as Open Fort — unchanged, already true), but every card was completely free to bond/consume on-color, which meant there was no real cost-benefit tension to a drafted pool at all ("right now I just use all the cards on my chao"). Reverses §5.6's original "Fruit is pooled, not per-color" decision and adds a real base cost to every card.
...
- **New `FRUIT_COST_BY_RARITY: Record<Rarity, number>`** (common 1, uncommon 2, rare 3, legendary 4 — first-draft placeholder, same tuning status as `DEFAULT_SPLASH_TAX`) — every Bond Card and Potion Card now costs this much Fruit **in its own color** to use, always, not just when off-identity. `bondCardWithSplashTax` (existing), the new `consumePotionWithCost`, and the new `awakenBondCardWithCost` (3x the base cost, fusing 3 copies at once) all charge this from the card's own color bucket via a shared `tryChargeFruit`/`buildCharges` helper pair, in addition to the existing splash tax if the card is off-color — **splash tax now specifically draws from the `colorless` bucket** rather than "any Fruit," giving Wildcard Fruit a real, distinct purpose instead of just describing one.
- **App-side** (`useGame.ts`): `bondBondCard`/`awakenBondCard`/`consumePotionCard` all route through the new cost-aware sim functions and surface a specific "needs X [color] Fruit (have Y)" message via the existing `actionMessage` banner (playtest-prep, §5.11 note if renumbered) on failure, naming exactly which shortfall (base cost, splash tax, or both) blocked the action. `EnvironmentPanel.tsx` now renders all 6 buckets (5 colors + Wildcard) at all times, even at 0, rather than a single "Fruit: N" line.
- Verified live via Playwright: a fresh Tournament-start grant correctly showed doubled per-color amounts plus the flat colorless bonus; an attempted off-color-for-the-player bond correctly blocked with an exact "needs 1 blue Fruit (have 0)" message; a same-run on-color bond succeeded and deducted exactly 1 Fruit from the correct color bucket, confirmed via both the Environment panel display and the Event Log's "(paid 1 black Fruit)" note. 135 sim tests (+9 over the prior count), all green; `pnpm -w build`/`typecheck`/`test` all green.
