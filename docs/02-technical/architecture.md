...
**Extended 2026-08-21 (roadmap.md Phase 5.9), per the user's direct request to "hook Traits into races":**
- `collectTriggerables` now also reads `chao.items` (equipped Item Cards whose effect is a real `TriggeredEffect`, not a passive `StaticModifier` — the latter is folded into base stats once at equip time, `chao/bonding.ts`'s `equipItem`, and never collected here). A real gap before this: `Chao.items` existed on the type since Phase 1, but nothing ever read it, so an equipped Item's trigger could never fire regardless of how it was authored.
- `fireTriggers` now enforces `TriggeredEffect.onceLimit: 'per_race'` via an optional `firedOnce: ReadonlySet<string>` parameter (keyed `${source}:${cardId}`) that it both reads and returns — `race.ts`'s `resolveRace` threads one instance of this set through every checkpoint call for the whole Race. This field existed since Phase 1 and was never enforced at all before this.
- `race.ts`'s `resolveRace` gained two new checkpoints: `stamina_below` (checked fresh after every Leg's Stamina deduction — declared "live" in `types.ts` since Phase 1 but never actually evaluated anywhere before this) and `race_end` (fires once at the very end, the counterpart to `race_start` — a brand-new `TriggerCondition` variant, added because White's Still Waters Trait had no way to check a finish-of-race condition at all).
- `race.ts` now also consumes `grantAlternateRoute` from a Leg's `leg_start` controlOps — declared in `types.ts` since the 2026-08-20 "special traversal" resolution but never read by any resolver before this. Gained a real `altStat: Stat` field (previously just `legType`/`description`) so a matching Leg's stat check swaps to a different stat at the same difficulty.
- `events/shared.ts`'s `applyEffectOps` still only narrates `grantFruit` as a `fruit_gained` event (it has no `Environment` reference, only a `Chao`) — but as of this same pass, `tournament/environment.ts`'s new `applyFruitEvents()` is what actually credits those events to `environment.fruit.colorless`, called from `useGame.ts` after every Race. Before this, `grantFruit` was flavor-only — narrated in the log, never actually paid out.

**Trait/Item bonding, added 2026-08-21 (roadmap.md Phase 5.9):** `Chao.traits`/`Chao.items` existed on the type since Phase 1, but there was no `bondTrait`/`equipItem` anywhere to ever populate them. New `chao/bonding.ts` functions: `bondTrait`/`unbondTrait` (capped at `MAX_TRAITS = 2` per GDD §3.5/§4.2, freely swappable, no Fruit refund on unbond — the cost was for the choice to run with it a while, not a rental fee) and `equipItem`/`unequipItem` (uncapped; applies/reverses a `StaticModifier`'s flat stat bonus immediately if the Item's effect is passive rather than triggered). Cost-aware wrappers `bondTraitWithCost`/`equipItemWithCost` in `tournament/environment.ts` reuse `chargeFruit`'s existing colorless-shortfall-fallback logic exactly — a Trait pays its base cost + splash tax like a Bond Card (its `color` field feeds `colorIdentity`/alignment the same way, per `derived.ts`); an Item pays a flat colorless cost only (`ItemCard.color` is always `'colorless'`, so there's no off-identity surcharge to speak of and no "own color" bucket to draw from first).
...
- **`Environment` state**: exactly 3 Habitat Slots (`(HabitatSlot | undefined)[]`, `undefined` = Open Fort), a list of still-unplaced drafted Habitat cards, a list of still-plantable drafted Seed cards, and a Fruit balance. **Superseded 2026-08-21 (playtest-prep) — that Fruit balance is now a per-color `FruitPool`, not a single number; see §5.10.**
- ~~Fruit is pooled, not per-color~~ — **reversed 2026-08-21, see §5.10.** (Was: a deliberate implementation-time simplification, since §4.4's splash tax was color-agnostic and nothing else consumed a per-color balance. A Habitat/Seed's color was meaningful for *display* only, not for what a bond could actually be paid with.)
...
### 5.10 Per-color Fruit and real card costs — implemented 2026-08-21 (roadmap.md playtest-prep)

Requested directly by the user after a live playtest: every card was completely free to bond/consume on-color, which meant there was no real cost-benefit tension to a drafted pool at all ("right now I just use all the cards on my chao"). Reverses §5.6's original "Fruit is pooled, not per-color" decision and adds a real base cost to every card. **Note: this section originally claimed habitat selection was "confirmed open... already true" — that turned out to be wrong; see this section's own follow-up note below, where it's actually fixed.**

- **`Environment.fruit` is now a `FruitPool`** (`Record<StatColor | 'colorless', number>`), not a single number. `slotFruitByColor()` breaks a Habitat slot's per-trigger yield down by color: the slot's own color gets `totalUnits − plantedSeedColors.length`, each planted Seed's color gets `+1` — since a slot's `seedSlots` is always exactly `totalUnits − 1`, capping conversions at `seedSlots` (already enforced by `canPlantSeed`) automatically leaves at least 1 native-color unit, with no separate clamp needed. This makes Seed color-conversion genuinely real for the first time (previously flavor/display only, §5.6).
- **`triggerFruitGain`** (after every race) is unchanged in rate, just writes into the correct color bucket now. **New `triggerInitialFruitGain`**, used exactly once — at the "continue to Tournament" transition, replacing the plain `triggerFruitGain` call that used to fire there — doubles the per-slot yield and adds a flat `+4` to the `colorless` bucket, per the user's explicit request ("double our current fruit generation after the initial draft... not after every race").
- **New `FRUIT_COST_BY_RARITY: Record<Rarity, number>`** (common 1, uncommon 2, rare 3, legendary 4 — first-draft placeholder, same tuning status as `DEFAULT_SPLASH_TAX`) — every Bond Card and Potion Card now costs this much Fruit **in its own color** to use, always, not just when off-identity. `bondCardWithSplashTax` (existing), the new `consumePotionWithCost`, and the new `awakenBondCardWithCost` (3x the base cost, fusing 3 copies at once) all charge this via a shared `chargeFruit` helper, in addition to the existing splash tax if the card is off-color — **splash tax draws from the `colorless` bucket** rather than "any Fruit," giving Wildcard Fruit a real, distinct purpose instead of just describing one.
- **App-side** (`useGame.ts`): `bondBondCard`/`awakenBondCard`/`consumePotionCard` all route through the new cost-aware sim functions and surface a specific "needs X Fruit total (have Y own-color + Z Wildcard)" message via the existing `actionMessage` banner on failure. `EnvironmentPanel.tsx` now renders all 6 buckets (5 colors + Wildcard) at all times, even at 0, rather than a single "Fruit: N" line.
- Verified live via Playwright: a fresh Tournament-start grant correctly showed doubled per-color amounts plus the flat colorless bonus; an attempted off-color-for-the-player bond correctly blocked with an exact shortfall message; a same-run on-color bond succeeded and deducted exactly 1 Fruit from the correct color bucket, confirmed via both the Environment panel display and the Event Log's "(paid 1 black Fruit)" note.

**Two more real bugs found and fixed, 2026-08-21, after the user actually played a round with the above:**

- **Habitat colors are now genuinely free-choice, not draft-dependent.** `draft/pool.ts`'s `drawBonusHabitat` (a random color, one guaranteed bonus Habitat card per pack) was, until now, the *only* way a slot ever got a color — the user ended up "stuck with 1 white and 2 black" with no say in it. New `setHabitatChoice(environment, slotIndex, color | undefined)` lets the player freely pick (and freely re-pick, right up until "Continue to Tournament") any color or Open Fort for each of the 3 slots directly — no card, no draft dependency.
- **`chargeFruit` (was `tryChargeFruit`/`buildCharges`) now lets colorless Fruit cover a shortfall in the base cost itself, not just splash tax** — a real bug: the base cost previously required the *full* amount from the card's exact color, and since only 3 Habitat slots exist for 5 colors, at least 2 colors are *always* stuck at 0 native Fruit forever. Any card in one of those colors — including a 3-copy Awakening — was therefore permanently unusable no matter how much Wildcard Fruit was banked, which is exactly what the user hit as "Awakening doesn't work." Fixed: the card's own color is drawn down first, colorless covers whatever's left short (for the base cost) plus any splash tax, matching the GDD's own pre-existing "Wildcard Fruit spends as any color" rule (§6.9) that splash tax alone used to honor. Result types gained `baseCostFromColorless`/`costFromColorless` so the app layer can log accurately which bucket actually paid for what.
- 142 sim tests (+7 over the prior count: `setHabitatChoice` behavior, and colorless-covers-a-shortfall regression tests for bonding, Awakening, and Potions).

**A third real bug, found immediately after: the "Drawn Habitat Cards" section (and its "Continue to Tournament" guard) was still there and actively blocking the player** — the free-choice picker above landed alongside the *existing* card-based placement UI, not instead of it, and `drawBonusHabitat` kept firing every round, so the player kept accumulating cards the guard demanded be resolved before Continue would enable. User report: "The Drawn habitat card section still exists but isnt part of the game anymore... i also cant go to the tournament - its blocking me." Fixed at the source rather than patched around: `draft/engine.ts`'s `startRound` no longer calls `drawBonusHabitat` at all (removed, not just unused) — `unplacedHabitats` is therefore always `[]`. `HabitatPlacementScreen.tsx` dropped the "Drawn Habitat Cards" section, the `canPlaceHabitatCard` guard, and the `onPlace` prop entirely; `useGame.ts`'s `placeHabitatCard` callback and its `HabitatCard` import were removed as genuinely dead app-layer code. `placeHabitatCard`/`canPlaceHabitatCard`/`drawBonusHabitat` themselves are untouched in the sim package and still pass their existing tests — kept dormant in case a future mechanic reintroduces a way to grow a Habitat to 2-star, per GDD §6.9's own note. `draft/engine.test.ts` updated: a completed draft's pool is now always exactly `packSize * 3` cards, no bonus habitats ever included.

Verified live via Playwright throughout: freely choosing and re-choosing all 3 slots' colors; an Awaken that would have been blocked under the old rule (0 native color) succeeded correctly paying from Wildcard Fruit; a planted Seed's color visibly appeared as its own Fruit bucket after the next race trigger; the "Drawn Habitat Cards" section confirmed absent and "Continue to Tournament" confirmed always enabled, reaching the Tournament screen cleanly. `pnpm -w build`/`typecheck`/`test` all green throughout.
