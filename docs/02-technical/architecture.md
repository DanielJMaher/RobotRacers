...
- **Seed planting**: `plantSeed()`/`canPlantSeed()`, symmetric to the above. Seeds are tracked in `Environment.availableSeeds` and removed on planting — unlike Bond/Regimen cards, which stay in the shared drafted pool forever and can be reused indefinitely (that's intentional stacking for Bond Cards; it would be an exploit for a scarce, one-time Seed).
