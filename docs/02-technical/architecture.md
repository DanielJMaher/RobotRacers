...
- 142 sim tests (+7 over the prior count: `setHabitatChoice` behavior, and colorless-covers-a-shortfall regression tests for bonding, Awakening, and Potions). Verified live: freely choosing and re-choosing all 3 slots' colors: an Awaken that would have been blocked under the old rule (0 native color) succeeded correctly paying from Wildcard Fruit; a planted Seed's color visibly appeared as its own Fruit bucket after the next race trigger, confirmed non-flavor. `pnpm -w build`/`typecheck`/`test` all green.
