...
- [ ] `chao/derived.ts`'s `recomputeDerived()`: color identity / alignment / species tag counts now fold over the full `bondedCards` history instead of `Object.values(bondSlots)`. New derived concept needed: a per-region visual blend (which creature-look currently dominates each of the 5 regions) — exact algorithm is an open question below, not yet designed.
...
- [ ] `GardenScreen.tsx`: the "Bond Slots" 4-entry list UI no longer makes sense (no fixed small slot set to show) — needs a new display, likely a per-region summary or a scrollable bonding history.
...
- [ ] **Content follow-up, tracked separately, not part of this phase's "done":** re-author Uncommon+ cards to actually carry the full multi-region, mixed-sign pattern (most currently only grant 1-2 positive stats on a single region) — a real design/balance pass, not a mechanical migration. Commons stay simple per the rarity-gated-complexity decision below.
...
| Final Body Region set — is **Legs/Arms/Back/Head/Torso** (5 regions, working default per GDD §3.5) the right count and split, or does it need adjusting once real multi-region content is authored? | 2.5 | Flagged explicitly open by the user when they gave the working default — not a placeholder guess, a genuine "come back to this." |
| Does Chao **coloring** (skin/fur color pattern, distinct from the 5-color card pie) mean anything — purely cosmetic fun, or does it interact with some mechanic? | 2.5 | New question, raised by the user alongside the Body Region correction. Not explored at all yet. |
| **TODO to reconsider:** card complexity is currently rarity-gated (Commons simple/single-region, Uncommon+ multi-region mixed-sign) — the user chose this explicitly but flagged it for revisiting, as opposed to *every* Bond Card carrying the full "Penguin" pattern from the start. | 2.5, 6 | Revisit once there's been a chance to author and playtest against the rarity-gated default. |
| Special traversal/shortcut-granting cards (the user's Elephant "walks the riverbed instead of swimming" example) — concrete mechanism not yet designed: a new EffectOp, or an extension of the existing Fly/Swim fork check (§5.1)? | 2.5 | Distinct from a stat boost — this is a card granting an alternate *route*, not a bigger number feeding the existing threshold check. |
| Per-region visual-blend algorithm: once a Chao has bonded many cards touching the same region, how is the region's displayed look actually computed — top-N dominant contributors, a true proportional blend, something else? How much history realistically renders before it becomes visual mush? | 2.5 | GDD §3.5 establishes the *requirement* (blend from full history, per region) without a concrete algorithm. |
