...
| Past-Tournament-winners pool mechanism — four separate sub-questions, all still open (GDD §6.5 spells these out in detail): **(a)** does the pool store the literal winner Chao (can recur as a named rival) or an abstract stat-template snapshot (only the *toughness* persists, not the individual)? **(b)** pool size/decay — capped with old entries aging out, or unbounded for the life of a save? **(c)** draw rate — fixed count, percentage chance per slot, or ramping with Tournament count? **(d)** scope — local to one save, or shared/global (which would double as the async-ghost multiplayer bridge, §7)? | 5 | Intent is clear ("preserve/escalate difficulty"), mechanism is not — needs (a) decided first since it changes what (b)–(d) even mean. |
| Kindergarten — does a Kindergarten-style event get a new home in the Tournament structure, and if so what does it actually do mechanically? See the explanation below the table. | 7 | Low priority; may simply be cut. |
