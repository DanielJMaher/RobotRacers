...
## Phase 2.5 — Fix the Bond Card model (cumulative, multi-region, mixed-sign)
...
- [ ] `types.ts`: rename `BondSlot` → `BodyRegion` with 5 values, **locked 2026-08-20**: **Legs, Arms, Back, Head, Torso**. `StatGrant` gains an optional `region: BodyRegion` field and must support negative `min`/`max` (already technically possible, just needs to become the norm rather than the Grinding Stone exception). `BondCard` drops its single `slot` field; gains `bodyMutations: Partial<Record<BodyRegion, string>>` so one card can carry a cosmetic mutation per region it touches.
- [ ] `Chao`: replace `bondSlots: Partial<Record<BondSlot, BondedCard>>` (4-slot exclusive record) with an unbounded, append-only history — e.g. `bondedCards: BondedCard[]`. `BondedCard.rolledGrants` entries carry their region tag through from the `StatGrant` they were rolled from.
- [ ] `chao/bonding.ts`'s `bondCard()`: remove all "check occupied slot → reverse previous card's contribution → replace" logic. New behavior: roll each grant (respecting sign), accumulate onto `chao.stats`, append the `BondedCard` to the history — full stop. `BondResult` loses `replacedCard` (nothing is ever replaced anymore).
- [ ] `chao/derived.ts`'s `recomputeDerived()`: color identity / alignment / species tag counts now fold over the full `bondedCards` history instead of `Object.values(bondSlots)`. New derived concept needed: a per-region visual look, **decided 2026-08-20 to be discrete top-contributor-wins** — per region, tally Species Tag contributions from every bonded card that touched it, and whichever tag has the highest count determines that region's `bodyMutation` asset (a simple count/max, not a rendered blend — see GDD §3.5 for why a true proportional blend is explicitly deferred).
- [ ] New `EffectOp` variant for special traversal/shortcut cards (**decided 2026-08-20**, working name `grantAlternateRoute`): a Chao clears a Leg via a wholly different condition than its normal stat check (the Elephant "walks the riverbed instead of swimming" example) — deliberately a separate mechanic from the existing Fly/Swim fork's threshold check (§5.1), not an extension of it. Exact field shape (which Leg type, what it lets the Chao ignore/substitute) TBD when this is actually implemented.
- [ ] Migrate all 91 authored cards (`packages/sim/src/cards/data/*.ts`) to the new shape — mechanical schema migration (per-grant region tags, `bodyMutations` records) so the codebase compiles and existing content stays valid. This is **not** the same task as re-authoring cards to actually carry the multi-region mixed-sign "Penguin" pattern — see the content follow-up below.
- [ ] `GardenScreen.tsx`: the "Bond Slots" 4-entry list UI no longer makes sense (no fixed small slot set to show) — needs a new display, likely a per-region summary (5 regions, each showing its current top-contributor look) or a scrollable bonding history.
- [ ] Update every existing test that references `bondSlots`/`BondSlot`/`replacedCard` (`bonding.test.ts`, `shared.test.ts`, `race.test.ts`, and `coreGardenSet.test.ts` if it checks slot-shaped fields).
...
**Done when:** `pnpm build`/`typecheck`/`test` all green against the new model; a Chao can bond 5+ cards across overlapping regions with both positive and negative grants and end up with the mathematically-correct accumulated stats (no replacement); the UI reflects a bonding history rather than 4 slots.

## Phase 3 — Tournament bracket core loop

The bracket itself, with no Environment Interludes or Breeding yet — just prove 24 entrants can play down to a ranked Final Race. Depends on Phase 2.5.
