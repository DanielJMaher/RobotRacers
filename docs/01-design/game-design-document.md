...
**Content follow-up — resolved 2026-08-21 (roadmap.md Phase 5.9):** the Bout-only-triggered keywords flagged below were reflavored into real Race-relevant effects, not left dormant or retired. Warthog Tusks' Knockback+ is now a Power gain on every cleared Leg; Jackrabbit Reflex's "always acts first" is now a permanent Run boost at Race start; Coral Turtle Shell's Bulwark is now a permanent Swim surge at Race start — see each card's own inline comment in `packages/sim/src/cards/data/*.ts` for the full reasoning per card. Every Trait Card (10 total) and every Bond Card keyword (~17 that used a dead trigger) across all 5 color files got the same treatment in the same pass, alongside a from-scratch bonding mechanism for Traits/Items (`chao/bonding.ts`'s `bondTrait`/`equipItem`, previously nonexistent — `Chao.traits`/`Chao.items` could never actually be populated by anything before this) and new engine hooks the rewrite depended on: `onceLimit: 'per_race'` enforcement, a real `stamina_below` Race checkpoint, a consumed `grantAlternateRoute` op (gained a real `altStat` field), a new `race_end` trigger, and `grantFruit` events actually crediting the Environment (previously narrated-only). Two cards (Blue's Tidewatcher's Eye — a draft-time peek with no Race-side equivalent at all; White's Guardian's Oath — a multi-Chao-board mechanic that doesn't fit this single-Chao design) were reflavored entirely rather than forced into a shape that only pretends to match their old text — full original-text preservation wasn't possible for those two specifically.
...
**A third bug, found immediately after the two above: the "Drawn Habitat Cards" section (and the "Continue to Tournament" guard requiring every placeable one be resolved first) was still there, now genuinely vestigial once habitat choice went free — and it was actively blocking the player from ever reaching the Tournament.** User report: "The Drawn habitat card section still exists but isnt part of the game anymore... i also cant go to the tournament - its blocking me." Root cause: `drawBonusHabitat` (the random per-pack bonus card) was still firing every round, so the player kept accumulating cards the guard demanded be placed — but with a free-choice slot already occupying a color, a mismatched bonus card often had nowhere valid left to go, permanently disabling Continue. Fixed by removing the mechanism at its source rather than patching around it: `draft/engine.ts`'s `startRound` no longer calls `drawBonusHabitat` at all, so no Habitat cards are ever drafted anymore. `HabitatPlacementScreen.tsx` dropped the "Drawn Habitat Cards" section and the guard entirely — "Continue to Tournament" is now always enabled. This also means 2-star Habitat combining (above) currently has no way to trigger — `placeHabitatCard`/`canPlaceHabitatCard`/`drawBonusHabitat` themselves are untouched and still tested, just unreachable from the app until some future mechanic reintroduces a 2nd Habitat Card.
...
| ~~Several already-authored cards (§4.2) have keywords written for a Bout that no longer exists — they're inert, not broken, but a player drafting them gets a dead-feeling card.~~ **Resolved 2026-08-21 (roadmap.md Phase 5.9).** | Undermines pillar #3 (rarity/power should be *discovered*, not a trap) if a drafted Rare or Legendary quietly does nothing. | Content follow-up pass reflavored every dead-triggered Trait and Bond Card keyword into a real, live Race-relevant effect — see §4.2's own note for the full list of what changed and why. Item Cards (previously 8 of 10 referencing entirely-removed pre-pivot systems) got the same treatment. |
